...
const SymptomAnalysis = () => {
  ...
  const [selectedSymptoms, setSelectedSymptoms] = useState([]); // store symptoms with underscores
  const [loading, setLoading] = useState(false);
  const [prediction, setPrediction] = useState(null); // { predicted_disease, confidence }
  const [error, setError] = useState(null);

  // Display symptom with spaces instead of underscores
  const formatSymptom = (symptom) => symptom.replace(/_/g, " ");

  // Filter symptoms by converting query spaces to underscores and checking
  ...
  // Add symptom (with underscores) to selectedSymptoms
  const addSymptom = (symptom) => {
    setSelectedSymptoms([...selectedSymptoms, symptom]);
    setQuery("");
  };

  // Remove symptom from selectedSymptoms
  const removeSymptom = (symptom) => {
    setSelectedSymptoms(selectedSymptoms.filter((s) => s !== symptom));
  };

  // Call API with selectedSymptoms (with underscores)
  ...
  return (
    <>
      {prediction ? (
        <AnalysisReport prediction={prediction} setPrediction={setPrediction} />
      ) : (
